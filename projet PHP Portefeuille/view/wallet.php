...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['user_id'])) {
    // Récupérer le montant et le commentaire du formulaire
    $montant = htmlspecialchars($_POST['montant']); // Sécuriser le montant pour éviter les injections XSS
    $commentaire = htmlspecialchars($_POST['commentaire']); // Sécuriser le commentaire
    ...
    if ($solde) {
        ...
        if ($montant < 0 && abs($montant) > $solde['solde']) {
            ...
            if ($stmt->execute()) {
                ...
                $sql_history = "INSERT INTO solde_history (user_id, montant, action, commentaire) VALUES (:user_id, :montant, :action, :commentaire)";
                ...
                $stmt_history->bindParam(':commentaire', $commentaire, PDO::PARAM_STR);
...
if (isset($_SESSION['user_id'])) {
    ...
    $sql_history = "SELECT sh.montant, sh.action, sh.commentaire, sh.created_at FROM solde_history sh
...
<head>
    ...
    <style>
        ...
        .history {
            display: none; /* Masquer l'historique par défaut */
            margin-top: 30px;
            padding: 10px;
            border: 1px solid #ddd;
    ...
    <?php else: ?>
        ...
        <div class="container">
            ...
            <form method="POST">
                ...
                <textarea name="commentaire" placeholder="Précisez l'origine de l'argent" rows="4" style="width: 80%;margin: 2px;"></textarea><br>
                <button type="submit">Mettre à jour le solde</button>
            </form>

            <button id="toggleHistoryButton">Afficher l'historique</button>
                ...
                <?php if (count($history) > 0): ?>
                    <?php foreach ($history as $entry): ?>
                        <div class="history-item">
                            ...
                            <p>Commentaire : <?php echo htmlspecialchars($entry['commentaire']); ?></p>
    ...
    <script>
        ...
        toggleHistoryButton.addEventListener('click', () => {
            if (historyContainer.style.display === 'none' || historyContainer.style.display === '') {
                ...
                toggleHistoryButton.textContent = 'Masquer l\'historique';
            } else {
                historyContainer.style.display = 'none';
                toggleHistoryButton.textContent = 'Afficher l\'historique';
            }
        });
